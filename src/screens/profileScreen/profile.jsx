...
const Profile = ({ theme, toggleMode, navigation, setInitializing }) => {
  ...
  const handleToggleModePress = () => {
    switchTheme({
      switchThemeFunction: () => {
        ...
        type: "fade", duration: 350, startingPoint: {
          cxRatio: 1, cyRatio: -1,
        },
      },
    });
  ...
  const compressAndUploadAvatar = async (image) => {
    ...
    try {
      ...
      if (compressionQuality) {
        // Resize the image
        const resizedImage = await ImageResizer.createResizedImage(image.uri, 512, // target width
          512, // target height
          "JPEG", compressionQuality, // compression quality
        );
  ...
  const styles = StyleSheet.create({
    ...
    body: {
      ...
      width: "100%", flexDirection: "row", justifyContent: "space-between", bottom: "2%",
    }, profileMainCardAuthBtn: {
      width: "100%", height: "100%", justifyContent: "center", alignItems: "center", borderRadius: 15,
    }, logoutOverlay: {
      backgroundColor: theme.colors.background, borderRadius: 15, width: wp(80), maxWidth: wp(92),
    }, profileMainCardBody: {
      ...
      flex: auth().currentUser.isAnonymous ? 2 : 3, justifyContent: "center", alignItems: "center",
      ...
      height: "auto", width: wp(100), backgroundColor: theme.colors.background, borderRadius: 15, marginTop: 12,
    ...
    btnStyle: {
      ...
      width: "46%", height: 42, borderRadius: 15,
    }, footerText: {
            ...
            <View style={styles.profileMainCard}>
              {auth().currentUser.isAnonymous && <>
                <View style={styles.profileMainCardHeader}>
                  <Text style={{
                    fontFamily: "Roboto-Black", fontSize: 24, color: theme.colors.accent, alignSelf: "flex-start",
                  }}>RoccaRent</Text>
                  <TouchableOpacity
                ...
                <View style={styles.profileMainCardBody}>
                  ...
                  <View style={styles.profileMainCardAuthBtnContainer}>
                    <ShadowedView style={[shadowStyle({
                      color: theme.colors.grey3, opacity: 1, radius: 4, offset: [0, 0],
                      ...
                      >
                        <Text
                          style={{
                            fontFamily: "Roboto-Bold", fontSize: 16, color: theme.colors.accentText,
                          }}>Вход</Text>
                      </Button>
                    </ShadowedView>
                    <ShadowedView style={[shadowStyle({
                ...
                <View style={styles.profileMainCardBody}>
                  <View>
                    <View style={{ width: 78, height: 78 }}>
                      {isAvatarLoading ? (<Skeleton circle width={"100%"} height={"100%"}
                  ...
                  {userdata.nickname ? <View>
                    <Text numberOfLines={1} style={{
                      fontFamily: "Roboto-Bold", fontSize: 20, color: theme.colors.text, alignSelf: "center",
                    }}>{userdata.nickname}</Text>
                    <Text numberOfLines={1} style={{
                      fontFamily: "Roboto-Bold", fontSize: 16, color: theme.colors.grey1, alignSelf: "center",
                    }}>{auth().currentUser.displayName}</Text>
                  </View> : <Text numberOfLines={1} style={{
                    fontFamily: "Roboto-Bold", fontSize: 18, color: theme.colors.text, alignSelf: "center",
                  }}>{auth().currentUser.displayName}</Text>}
                </View>
              </>}
            </View>
        ...
        {!auth().currentUser.isAnonymous && <ShadowedView style={[styles.profileDataContainer, shadowStyle({
          ...
          <Button
            ...
            <View style={{ flex: 1 }}>
              ...
              }}>Имя{"\t"}{!userdata.nickname && <Badge value={"Придумайте никнейм"} status={"warning"} />}</Text>
              <Text numberOfLines={1} style={{
                fontFamily: "Roboto-Regular", color: `${theme.colors.text}AA`, fontSize: 14, marginStart: 12,
